...
import { SOUTHFL_TABS } from '../utils/prompts';
import { useTabData } from '../hooks/useTabData';

export default function SouthFLSection({ apiKey }) {
  const [activeTab, setActiveTab] = useState(SOUTHFL_TABS[0].id);
  const { data, loading, errors, loadTab, isLoaded } = useTabData();

  const currentTab = SOUTHFL_TABS.find((t) => t.id === activeTab);

  useEffect(() => {
    if (!isLoaded(activeTab) && !loading[activeTab]) {
      const tab = SOUTHFL_TABS.find((t) => t.id === activeTab);
      loadTab(activeTab, tab.prompt(), apiKey);
    }
  }, [activeTab, apiKey]);

  const handleRefresh = (tabId) => {
    const tab = SOUTHFL_TABS.find((t) => t.id === tabId);
    ...
    >
      {isLoading && (
        <div className="loading-container">
          ...
          <div className="loading-text">Searching the web for South Florida intel...</div>
        </div>
      )}
